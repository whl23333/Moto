...
class HDF5Dataset_for_MotoGPT_CALVINLike(Dataset):
    def __init__(
        ...
        # per-instance file cache (each DataLoader worker gets its own Dataset instance)
        self._file_cache = {}
    ...
    def _get_file(self, file_path):
        ...
        Each Dataset instance lives inside a single worker process, so this
        cache is per-worker and avoids repeatedly opening/closing files.
        """
        f = self._file_cache.get(file_path)
        if f is None or not getattr(f, "id", None) or not f.id.valid:
    ...
    def __getitem__(self, idx):
        ...
        while attempts < self.max_attempts:
            try:
                ...
                # open file via per-instance cache (reused within worker)
