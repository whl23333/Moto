...
class MotoGPT_Trainer_Metaworld:
    def __init__(
        ...
        optimizer = torch.optim.AdamW(moto_gpt.parameters(), lr=lr_max, weight_decay=weight_decay, fused=True)
        # total_prints_per_epoch = len(train_dataloader.dataset) // (print_steps * bs_per_gpu * accelerator.num_processes) # len(train_dataloader.dataset) too small
        # scheduler = get_cosine_schedule_with_warmup(
        #     optimizer, 
        #     num_warmup_steps=min(num_warmup_epochs*total_prints_per_epoch, 5000000 // (print_steps * bs_per_gpu * accelerator.num_processes)),
        #     num_training_steps=num_epochs*total_prints_per_epoch,
        ...
        if latent_motion_tokenizer is not None:
            latent_motion_tokenizer = latent_motion_tokenizer.to(accelerator.device)
            latent_motion_tokenizer.eval()
            print("acelerator device: ", accelerator.device)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs+1):
            ...
            # rgb_initial, rgb_future, task, action_arm, action_gripper, mask = next(iter(train_dataloader))
            # lang_input_ids = self.lang_tokenizer(task, return_tensors='pt')['input_ids'] # shape: (b, t)
            # batch = {
            #     'rgb_initial': rgb_initial.to(self.device), # shape: (b, 1, c, h, w)
            #     'rgb_future': rgb_future.to(self.device), # shape: (b, 1, c, h, w)
            #     'lang_input_ids': lang_input_ids.to(self.device), # shape: (b, t)
            #     'actions': torch.cat([action_arm, action_gripper], dim=-1).to(self.device), # shape: (b, 7, 4)
            #     'mask': mask.to(self.device), # shape: (b, 7)
            # }
            for batch in self.train_dataloader:
                with self.accelerator.accumulate(self.moto_gpt):
                    ...
                    self.accelerator.backward(loss['total_loss'])
                    self.optimizer.step()
                print(f"step: {step}, loss: {loss['total_loss']}")
                self.scheduler.step()
                
                if step % self.print_steps == 0:
                    self._log_progress(loss, step, epoch)
    ...
    def _log_progress(self, loss, step, epoch):
        text = 'Train Epoch: {} Step: {} '.format(epoch, step)
        for key in loss:
            text = text + ' {}_loss: {:.5f}'.format(key, loss[key])
        self.print(text)
        if self.is_main:
            for key in loss:
                self.writer.add_scalar(key+'_loss', loss[key], step)
            self.writer.add_scalar("learning rate", self.scheduler.get_last_lr()[0], step)
    @torch.no_grad()
    def eval_latent_motion_gen(self, visualization_dir):
        os.makedirs(visualization_dir, exist_ok=True)
        ...
        rgb_initial, rgb_future, task, action_arm, action_gripper, mask = next(self.eval_dataloader)
        lang_input_ids = self.lang_tokenizer(task, return_tensors='pt', padding=True, truncation=True)['input_ids'].to(self.device) # shape: (b, L)
        batch = {
            'rgb_initial': rgb_initial, # shape: (b, 1, c, h, w)
            'rgb_future': rgb_future, # shape: (b, 1, c, h, w)
            'lang_input_ids': lang_input_ids, # shape: (b, L)
            'actions': torch.cat([action_arm, action_gripper], dim=-1), # shape: (b, 1, 7, 4)
            'mask': mask, # shape: (b, 1)
            'lang': task,
        }
...
class MotoGPT_Trainer_RLBench:
    def __init__(
        ...
        latent_motion_tokenizer,
        rgb_preprocessor,
        lang_tokenizer, # previous input: (b, L) current input: (b, 2, 53, 512)
        train_dataloader,
        eval_dataloader,
        save_path,
        save_epochs=1,
        ...
        self.bs_per_gpu = bs_per_gpu
        self.pred_binary_gripper_action = pred_binary_gripper_action
        self.use_paired_loss = use_paired_loss
        ...
        for epoch in range(self.num_epochs+1):
            ...
            for batch in self.train_dataloader:
                ...
                with self.accelerator.accumulate(self.moto_gpt):
                    ...
                    self.optimizer.zero_grad()
                    # loss = self.calculate_loss(batch, train=True)
                    loss = self.calculate_multi_loss(batch, train=True)
                    self.accelerator.backward(loss['total_loss'])
                    self.optimizer.step()
                ...
                if (batch_idx+1) % self.print_steps == 0:
                    ...
                    with torch.no_grad():
                        ...
                        loss = self.calculate_multi_loss(eval_batch, train=True)
                        for key in eval_log_loss:
                            eval_log_loss[key] = loss[key].detach()
                ...
                if step % self.save_steps == 0: #(batch_idx+1)  % self.save_steps == 0:
                    self.accelerator.wait_for_everyone()
                    save_dir = os.path.join(self.save_path, f'temp_epoch_{epoch}_step_{step}')
    ...
    def eval_latent_motion_gen(self, visualization_dir):
        ...
        # rgb_seq = self.rgb_preprocessor(orig_rgb_seq, train=True)
        rgb_initial = rgb_seq[:,:1,0] # （b, 1, c, h, w）

        # b, t, c, h, w = batch['rgb_future'].shape
        # b, t, c, h, w = rgb_seq.shape
        b, t, cameras, c, h, w = rgb_seq.shape
        t = t - 1
        if self.use_paired_loss:
            gt_latent_motion_ids = self.latent_motion_tokenizer(
                cond_pixel_values1 = rgb_seq[:, :-1, 0].reshape(-1, c, h, w),
                target_pixel_values1 = rgb_seq[:, 1:, 0].reshape(-1, c, h, w),
                cond_pixel_values2 = rgb_seq[:, :-1, 0].reshape(-1, c, h, w),
                target_pixel_values2 = rgb_seq[:, 1:, 0].reshape(-1, c, h, w),
                return_motion_token_ids_only=True
            )
            
            recons_rgb_future = self.latent_motion_tokenizer.decode_image(
                cond_pixel_values=rgb_seq[:, :-1, 0].reshape(-1, c, h, w),
                given_motion_token_ids=gt_latent_motion_ids
            )["recons_pixel_values"]
        else:
            gt_latent_motion_ids = self.latent_motion_tokenizer(
                cond_pixel_values=rgb_seq[:, :-1, 0].reshape(-1, c, h, w),
                target_pixel_values=rgb_seq[:, 1:, 0].reshape(-1, c, h, w),
                return_motion_token_ids_only=True
            )
            recons_rgb_future = self.latent_motion_tokenizer.decode_image(
                cond_pixel_values=rgb_seq[:, :-1, 0].reshape(-1, c, h, w),
        ...
        gt_latent_motion_ids = gt_latent_motion_ids.reshape(b, t, -1) # (b, 1, 8)
        recons_rgb_future = recons_rgb_future.reshape(b, t, c, h, w)
        recons_rgb_future = self.rgb_preprocessor.post_process(recons_rgb_future)
        ...
        for i in range(b):
            ...
            visualize_latent_motion_gen(
                lang_goal=batch['task'][i][0],
                orig_video=orig_rgb_seq[i, :, 0], 
                decoding_mode2preds=ith_decoding_mode2preds,
                path=os.path.join(visualization_dir, f"{self.process_index}-{i}")
            )
    
    def calculate_multi_loss(self, batch, train):
        loss = {}
        cameras = batch['rgbs'].shape[2]
        for i in range(cameras):
            batch_i = {
                'rgb_initial': batch['rgbs'][:, :1, i], # shape:(b 1 c h w)
                'rgb_future': batch['rgbs'][:, 1:, i], # shape:(b 1 c h w)
                'instr': batch['instr'][:, 0], # shape:(b 53 512)
                'mask': batch['mask'], # shape:(b 1)
                'latent_mask': batch['mask'], # shape:(b 1)
                'actions': batch['trajectory'][:, :1] # shape:(b 1 T 8)
            }
            loss_i = self.calculate_loss(batch_i, train=train)
    ...
    def calculate_loss(self, batch, train):
        ...
        if self.moto_gpt_config.get('pred_discrete_arm_action', False): # NOTE: calculate crosee_entropy loss for discrete arm action
            action_arm_loss_func = cross_entropy
            gt_action_arm = batch['actions'][..., :7].long()
        else:
            action_arm_loss_func = F.smooth_l1_loss
            gt_action_arm = batch['actions'][..., :7]
