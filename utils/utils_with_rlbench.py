...
class Actioner_motogpt:
    ...
    def __init__(
        ...
        self._policy.eval()
        self.use_camera_number = use_camera_number
    ...
    def predict(self, rgbs, pcds, gripper,
        ...
        if self._predict_trajectory:
            print('Predict Trajectory')
            inputs={
                'rgb': rgbs[:, -1, self.use_camera_number].unsqueeze(1), # bs, 1, 3, H, W
                'language': self._instr,
                'attention_mask': torch.ones(rgbs.size(0), 1).to(rgbs.device),
                'latent_motion_ids': None,
                'latent_mask': torch.ones(rgbs.size(0), 1).to(rgbs.device),
                'train': False
            }
            pred = self._policy(**inputs)
            output["trajectory"] = torch.cat([pred['arm_action_preds'], pred['gripper_action_preds']], dim=-1) # bs, 1, 2, 8
        else:
            print('Predict Keypose')
            pred = self._policy(
                rgbs[:, -1],
